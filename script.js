...
function initSimpleTopLineHoverCards() {
  ...
  document.querySelectorAll('h2, h3, h4').forEach((heading) => {
    if (!targetHeadings.has(heading.textContent.trim())) return;
    const card = heading.closest('article, .card, .panel, .form-card, .dashboard-card, div');
    card?.classList.add('top-line-hover-card');
  });
}

ensureDefaultAccounts();
renderAll();
initSimpleTopLineHoverCards();

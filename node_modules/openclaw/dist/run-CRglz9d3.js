...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-BTtQ75qF.js";
import { t as createSubsystemLogger } from "./subsystem-Bhq-McUP.js";
import "./globals-CX0Bl8ZU.js";
import { t as isContainerEnvironment } from "./container-environment-DxZhL--1.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-dUoGHCll.js";
...
import { n as setGatewayWsLogStyle } from "./ws-logging-B4Dx0Kmj.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-BWTg_mt-.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-LM1BOphD.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-r2flTmZW.js");
...
async function readGatewayStartupConfig(params) {
	...
	if (snapshot?.exists && !snapshot.valid) {
		...
		if (await params.startupTrace.measure("cli.config-recovery", () => recoverConfigFromLastKnownGood({
			...
			try {
				const { writeRestartSentinel } = await import("./restart-sentinel-DjoHqD7Z.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-KQfQJ3tQ.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-DwSc-oEt.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-WVBYLG82.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-BjqoKfkX.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-Cmhg_MCu.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-A3MeJ03O.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-BMrpdkrg.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-lAKoU_rO.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-D9KUNrML.js");

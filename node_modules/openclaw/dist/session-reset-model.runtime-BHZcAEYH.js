...
import { n as resolveModelDirectiveSelection, r as resolveModelRefFromDirectiveString, t as modelKey } from "./model-selection-directive-Cbqrx0C6.js";
...
async function resolveResetFallbackModels(params) {
	if (params.agentId) {
		const { resolveAgentModelFallbacksOverride } = await import("./agent-scope-JzdWovLn.js");
...
async function buildResetAllowedModelKeys(params) {
	if (Object.keys(params.cfg.agents?.defaults?.models ?? {}).length > 0 || params.cfg.models?.providers) {
		const { buildAllowedModelSetWithFallbacks } = await import("./model-selection-shared-DbY0pXgn.js");
...
function applySelectionToSession(params) {
	...
	if (storePath) import("./sessions-BPZefjgn.js").then(({ updateSessionStore }) => updateSessionStore(storePath, (store) => {

...
function hasStalePersistedPluginMetadata(index) {
	return index.plugins.some((plugin) => {
		if (!hasOptionalMissingPluginManifestFile(plugin)) if (fileSignatureMatches(plugin.manifestPath, plugin.manifestFile) === false) {
		...
		if (fileSignatureMatches(packageJsonPath, plugin.packageJson.fileSignature) === false) return hashExistingFile(packageJsonPath) !== plugin.packageJson.hash;

...
import { r as createConfigIO } from "./io-BcjzQKwO.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-CObL9KOW.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-D0p4NsP6.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-B73Mbn_g.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-Bkfev99t.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-ByWMCEzg.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CttJs1RN.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-CWBNRjEt.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-BS9o8UCQ.js"), import("./setup-registry-ChTL0P8A.js")]);
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-Nqe-OK2G.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DFMApzxE.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-CQfqeLNl.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-CQfqeLNl.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-D7O_adcg.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-D8pCsZ94.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C3TMah-Z.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DgsF2kCr.js");
		const { setupChannels } = await import("./onboard-channels-BbACi2jb.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-DtpR04V7.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-1DugJOLb.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-BuJm3WZB.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-C89U9oEF.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-C05zJBuu.js");

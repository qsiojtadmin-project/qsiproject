...
import { i as normalizeEnv, t as isTruthyEnvValue } from "./env-DzjAzpl4.js";
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-MSU-txfy.js"), import("./git-commit-DfPbea4K.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		if (!(deps.outputPrecomputedRootHelpText ?? (await import("./root-help-metadata-htJ4DDbG.js")).outputPrecomputedRootHelpText)()) {
			const { outputRootHelp } = await import("./root-help-C6rJPVnh.js");

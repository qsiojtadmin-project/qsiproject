...
import { r as createHttp1ProxyAgent } from "./undici-runtime-Ca1ojMYw.js";
import { r as resolveDebugProxySettings, t as applyDebugProxyEnv } from "./env-BlWX7jMF.js";
import { c as closeDebugProxyCaptureStore, i as initializeDebugProxyCapture, l as getDebugProxyCaptureStore, r as finalizeDebugProxyCapture } from "./runtime-CmqAKMVV.js";
import { t as fetchWithRuntimeDispatcher } from "./runtime-fetch-Ded514Nk.js";
import { t as buildDebugProxyCoverageReport } from "./coverage-CR4c_LyN.js";
...
function formatProxyCheckLine(check) {
	const icon = check.ok ? "✓" : "✗";
	const paddedKind = check.kind.padEnd(7, " ");
	const status = check.status === void 0 ? "" : ` HTTP ${check.status}`;
...
function formatProxyValidationText(result) {
	...
	if (result.config.errors.length > 0) {
		lines.push("", "Problems");
		for (const error of result.config.errors) lines.push(`  - ${error}`);
	}
	...
	if (nextSteps.length > 0) {
		...
		for (const nextStep of nextSteps) lines.push(`  ${nextStep}`);
	}
	return `${lines.join("\n")}\n`;
}
async function runProxyValidateCommand(opts) {
	...
	process.stdout.write(opts.json === true ? `${JSON.stringify(outputResult, null, 2)}\n` : formatProxyValidationText(outputResult));
